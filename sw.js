...
const CORE=['/','/index.html?v=205','/atleta.html?v=205','/athlete-start.html?v=205','/manifest.json?v=205','/athlete-manifest.json?v=205','/logo.png?v=205'];
...
self.addEventListener('fetch',event=>{
 ...
 if(req.mode==='navigate'){
  event.respondWith((async()=>{try{const fresh=await fetch(req,{cache:'no-store'});cachePut(req,fresh);return fresh}catch(_){const exact=await caches.match(req);if(exact)return exact;const athlete=url.pathname.endsWith('/atleta.html')||url.pathname.endsWith('/athlete-start.html');return (await caches.match(athlete?'/athlete-start.html?v=205':'/index.html?v=205'))||Response.error()}})());return;
 }
 if(url.origin===self.location.origin){event.respondWith((async()=>{try{const fresh=await fetch(req);cachePut(req,fresh);return fresh}catch(_){return (await caches.match(req))||Response.error()}})());return;}
